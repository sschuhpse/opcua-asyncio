...
async def make_structure_code(server,data_type, struct_name, sdef):
    """
...
"""
    ...
    for sfield in sdef.Fields:
        ...
        if sfield.DataType.NamespaceIndex == 0 and sfield.DataType.Identifier in ua.ObjectIdNames:
            ...
            if sfield.DataType.NamespaceIndex == 3 and sfield.DataType.NodeIdType == ua.NodeIdType.FourByte:
                uatype = await ua_utils.get_base_data_type(server.get_node(sfield.DataType))
                if uatype.nodeid.NamespaceIndex == 0 and uatype.nodeid.Identifier in ua.ObjectIdNames:
                    if uatype.nodeid.Identifier == 24:
                        uatype = "Variant"
                    elif uatype.nodeid.Identifier == 22:
                        uatype = "ExtensionObject"
                    else:
                        uatype = ua.ObjectIdNames[uatype.nodeid.Identifier]
            else:
...
async def _generate_object(server:Union["Server","Client"],name, sdef, data_type=None, env=None, enum=False, option_set=False):
    """
    ...
    if enum:
        ...
        code = await make_structure_code(server,data_type, name, sdef)
...
async def load_custom_struct(node: Node) -> Any:
    ...
    env = await _generate_object(node.server,name, sdef, data_type=node.nodeid)
    struct = env[name]
    ua.register_extension_object(name, sdef.DefaultEncodingId, struct, node.nodeid)
    return env[name]

def sort_types(dtypes):
    # TODO: change algorithm to efficient topological sort algorithm
    for z in range(len(dtypes)):
        stable=True
        for x in range(len(dtypes)):
            for y in range(len(dtypes)):
                if x != y:
                    if dtypes[x] < dtypes[y] and x > y or dtypes[y] < dtypes[x] and y > x:
                        dtypes[x], dtypes[y] = dtypes[y], dtypes[x]
                        stable=False
        if stable:
            return z
...
async def load_data_type_definitions(server: Union["Server", "Client"], base_node: Node = None, overwrite_existing=False) -> None:
    ...
    test=sort_types(dtypes)
    for dts in dtypes:
        try:
            env = await _generate_object(server,dts.name, dts.sdef, data_type=dts.data_type)
...
async def load_enums(server: Union["Server", "Client"], base_node: Node = None, option_set: bool = False) -> None:
    ...
    for desc in await base_node.get_children_descriptions(refs=ua.ObjectIds.HasSubtype):
        ...
        if not edef:
            continue
        env = await _generate_object(server,name, edef, enum=True, option_set=option_set)
